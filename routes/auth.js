...
// post. A token granted before these were added â€” or with the box un-ticked
// on the consent screen â€” can publish fine but never collects analytics, so
...
// Status â€” which platforms are connected + display names, for the logged-in user
...
router.get('/x/callback', async (req, res) => {
  if (!req.session.userId) return fail(res, 'Session expired â€” please log in and try connecting again')
...
// â”€â”€ Facebook + Instagram (single OAuth flow) â”€â”€
router.get('/facebook', requireAuth, (req, res) => {
  const state = crypto.randomBytes(16).toString('hex')
  req.session.fbState = state
  console.log('[facebook OAuth start] state:', state)
  console.log('[facebook OAuth start] session ID:', req.sessionID)
  console.log('[facebook OAuth start] userId:', req.session.userId)
  req.session.save((err) => {
    if (err) {
      console.error('[facebook OAuth start] session save error:', err)
      return fail(res, 'Could not save OAuth session')
    }
    console.log('[facebook OAuth start] session saved')
    console.log('[facebook OAuth start] redirect:', facebook.getAuthUrl(state))
    res.redirect(facebook.getAuthUrl(state))
  })
})
router.get('/facebook/callback', async (req, res) => {
  console.log('[facebook OAuth callback] query state:', req.query.state)
  console.log('[facebook OAuth callback] session fbState:', req.session?.fbState)
  console.log('[facebook OAuth callback] session ID:', req.sessionID)
  console.log('[facebook OAuth callback] session userId:', req.session?.userId)

  if (!req.session.userId) return fail(res, 'Session expired â€” please log in and try connecting again')
  if (req.query.state !== req.session.fbState) {
    console.error('[facebook OAuth] STATE MISMATCH')
    console.error('  query.state =', req.query.state)
    console.error('  session.fbState =', req.session?.fbState)
    return fail(res, 'State mismatch')
  }
  try {
    ...
    // pages, not necessarily the first one â€” check them all rather than
    ...
    if (instagramAccounts.length) {
      ...
      // No Instagram saved â€” almost always means no Page has an Instagram
      ...
      console.log(`[instagram] no linked Instagram professional account found across ${pages.length} page(s) â€” not connecting Instagram`)
    }
    res.send(SUCCESS_HTML)
...
// â”€â”€ Facebook Data Deletion Callback â”€â”€
// Required by Facebook for apps using Facebook Login.
// Facebook sends a signed_request identifying its own user_id â€” no session
...
// Deletion status page â€” shown to users who want to confirm their data was deleted
