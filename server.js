...
async function start() {
  ...
  const sslKey = path.join(__dirname, 'localhost+2-key.pem')
  const sslCert = path.join(__dirname, 'localhost+2.pem')
  ...
  httpsServer.listen(PORT, () => {
    console.log(`\n🚀  Flixty backend → https://localhost:${PORT}`)
    console.log(`🔑  OAuth callbacks use BASE_URL=${process.env.BASE_URL || `https://localhost:${PORT}`}`)
